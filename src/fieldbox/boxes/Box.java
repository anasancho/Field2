 ...
 */
public class Box implements fieldlinker.AsMap, HandlesCompletion, ProxyObject {
...
//		if (!box.properties.has(q)) return "";
    ...
    public int banana(int x) {
        System.out.println(" called banana " + x);
        return x + 10;
    }
        ...
        if (key.equals("banana")) {
            return new ProxyExecutable() {
                ...
                public Object execute(Value... arguments) {
                    return banana(arguments[0].asInt());
                }
            };
        }
    ...
    public Object getMemberKeys() {
        return properties.getMap().keySet().stream().map(x -> x.getName()).toArray();
    }
